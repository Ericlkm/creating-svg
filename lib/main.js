    ...
    return inquirer
      .prompt([
...

        {
          type: "input",
          name: "text",
          message: "write text in shape, must only contain (4) characters!",
        },
      ...
      .then(({ shape, shapeColor, text, textColor }) => {
        ...
        return fs.writeFile("logo.svg", svg.render(), (err) => {
          if (err) throw err;
          console.log(`\x1b[32m Created SVG Logo!\x1b[0m`);
        });
      })

      .then(() => {
        console.log(`\x1b[36m  Creating SVG Logo Created!\x1b[0m`);
      });
  }
}
